...
import com.example.imdbapp.common.utils.JsonUtils
import com.example.imdbapp.data.sources.api.model.response.ErrorResponse
...
object ErrorMapper {
    ...
    private fun <T> checkAndMapApiError(response: Response<T>): RetrofitResult<T> {
        return if (response.isSuccessful) {
            ...
            val errorResponse: ErrorResponse? = JsonUtils.convertJsonStringToObject<ErrorResponse>(response.errorBody()?.string())
            RetrofitResult.Failure(mapErrorCode(errorResponse?.statusCode ?: response.code(), errorResponse?.message))
        }
    }
}
